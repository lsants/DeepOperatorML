...
from src.modules.model.components import orthonormal_trunk, trainable_trunk
...
class TwoStepHelper:
    ...
    def compute_synthetic_targets(self, model: 'DeepONet') -> tuple[torch.Tensor]:
        ...
        if R is None or A is None:
            raise ValueError(
                "R or A matrix are not available; ensure trunk phase decomposition is complete.")

        # If there are multiple trunks (size of R = n * number of basis functions), merge the A matrix
        if R.shape == (n * K, n * K):
            blocks = [R[i * K: (i + 1) * K, i * K: (i + 1) * K]
                      for i in range(n)]
            R = torch.cat(blocks, dim=1)
            targets = tuple(model.output_handling.forward(
                model, branch_input=A, trunk_input=R))
        else:
            targets = model.output_handling.forward(
                model, branch_input=A, trunk_input=R)
        return targets
    ...
    def compute_loss(self,
        ...
        if not model.training_strategy.inference:
            if phase == "trunk":
                targets = tuple(batch[key]
                                for key in training_params["TARGETS"])
            elif phase == "branch":
    ...
    def compute_errors(self, outputs: tuple[torch.Tensor],
        ...
        if not error_norm:
            raise MissingSettingError(
                "Norm for error computation is undefined. Check .yaml file.")
        ...
        if not model.training_strategy.inference:
            if phase == "trunk":
                ...
                for key, target, pred in zip(training_params["TARGETS"], targets, outputs):
                    ...
                    norm_e = torch.linalg.vector_norm(
                        target - pred, ord=error_norm)
                    errors[key] = (
                        norm_e / norm_t).item() if norm_t > 0 else float("inf")
                return errors
            else:
                raise ValueError("Invalid training phase.")
        else:
    ...
    def compute_trained_trunk(self, model, training_params: dict, trunk_input: torch.Tensor) -> torch.Tensor:
        ...
        """
        pretrained = self.decomposition_helper.decompose(
            model, training_params, trunk_input)
        return pretrained
