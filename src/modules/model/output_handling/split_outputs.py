...
import logging
import torch
from .base import OutputHandling, OutputHandler
...
class SplitOutputHandler(OutputHandler):
    def compute(self, branch_out, trunk_out) -> torch.Tensor | Literal[0]:
        # Split and compute each output
        return sum(b * t for b, t in zip(branch_out, trunk_out))
    
    def validate_dimensions(self, branch_dim, trunk_dim):
        if branch_dim != trunk_dim * self.num_outputs:
