...
class ModelFactory:    
    ...
    def create_model(model_params: dict[str, Any], dataset_params: dict[str, Any], **kwargs: Any) -> DeepONet:
        ...
        branch_config, trunk_config = ModelFactory._config_components(model_params, dataset_params)
    ...
    def initialize_model(trained_model_config: dict[str, Any], dataset_config: dict[str, Any],device: str, **kwargs) -> DeepONet:
        ...
        if trained_model_config["TRAINING_STRATEGY"] == 'two_step':
            ...
            model = ModelFactory.create_model(model_params=trained_model_config,
    ...
    @staticmethod
    def _config_components(model_params: dict[str, Any], dataset_params: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
        branch_arch_key = model_params['BRANCH_ARCHITECTURE'].lower()
        branch_config = {
        ...
        activation_function_key = model_params['BRANCH_ACTIVATION']
        if ActivationFactory.has_activation(activation_key=activation_function_key):
            branch_config['activation'] = ActivationFactory.get_activation(activation_function_key)
        else:
                if branch_arch_key == 'kan':
                    branch_config['degree'] = model_params.get('BRANCH_DEGREE')
                else:
                    raise ValueError("Activation {activation_function_key} not found for arch {branch_arch_key}")

        trunk_arch_key = model_params['TRUNK_ARCHITECTURE'].lower()
        trunk_config = {
        ...
        if ActivationFactory.has_activation(activation_key=activation_function_key):
            trunk_config['activation'] = ActivationFactory.get_activation(activation_function_key)
        else:
                if trunk_arch_key == 'kan':
                    trunk_config['degree'] = model_params.get('BRANCH_DEGREE')
                else:
                    raise ValueError("Activation {activation_function_key} not found for arch {trunk_arch_key}")

        return branch_config, trunk_config
