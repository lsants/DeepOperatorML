...
def plot_basis(coords, basis, strategy, **kwargs):
    ...
    if coord_labels is not None and 'plot_dims' in coord_labels:
        ...
        if len(dims) < 2:
            raise ValueError(
                "At least two coordinate arrays are required for plotting 2D basis functions.")
    ...
    for ch in range(n_channels):
        ...
        if label_mapping is not None and label_mapping != '':
            axs[ch].set_title(
                f"Channel {label_mapping[ch]}, vector {index}", fontsize=12)
