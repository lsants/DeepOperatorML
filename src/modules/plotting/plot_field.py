...
def plot_2D_field(coords: dict[str, np.ndarray], truth_field: np.ndarray | None = None, pred_field: np.ndarray | None = None, **kwargs) -> plt.figure:
    ...
    if np.all(coord1 >= 0):
        if truth_field is not None:
            truth_mesh = np.concatenate(
                (np.flip(truth_mesh[1:], axis=0), truth_mesh), axis=0)
        if pred_field is not None:
            pred_mesh = np.concatenate(
                (np.flip(pred_mesh[1:], axis=0), pred_mesh), axis=0)
    ...
    if is_complex:
        ...
        if ncols == 3:
            ...
            c0 = ax[0, 0].contourf(
                X, Y, pred_real, cmap='viridis', norm=norm_real)
            ...
            c1 = ax[0, 1].contourf(
                X, Y, truth_real, cmap='viridis', norm=norm_real)
            if labels is not None:
            ...
            c2 = ax[0, 2].contourf(X, Y, np.abs(
                truth_real - pred_real), cmap='viridis')
            if labels is not None:
        ...
        if ncols == 3:
            ...
            c3 = ax[1, 0].contourf(
                X, Y, pred_imag, cmap='viridis', norm=norm_imag)
            if labels is not None:
            ...
            c4 = ax[1, 1].contourf(
                X, Y, truth_imag, cmap='viridis', norm=norm_imag)
            if labels is not None:
            ...
            c5 = ax[1, 2].contourf(X, Y, np.abs(
                truth_imag - pred_imag), cmap='viridis')
            if labels is not None:
