...
def prepare_batch(batch: dict[str, torch.Tensor], training_params: dict[str, any]) -> dict[str, torch.Tensor]:
    """
    ...
    xb_scaler = Scaling(
        min_val = training_params['NORMALIZATION_PARAMETERS']['xb']['min'],
        max_val = training_params['NORMALIZATION_PARAMETERS']['xb']['max']
    )
    xt_scaler = Scaling(
        min_val = training_params['NORMALIZATION_PARAMETERS']['xt']['min'],
        max_val = training_params['NORMALIZATION_PARAMETERS']['xt']['max']
    )

    if training_params['INPUT_NORMALIZATION']:
    ...
    for key in training_params['OUTPUT_KEYS']:
        scaler = Scaling(
            min_val = training_params['NORMALIZATION_PARAMETERS'][key]['min'],
            max_val = training_params['NORMALIZATION_PARAMETERS'][key]['max']
        )
        if training_params['OUTPUT_NORMALIZATION']:
    ...
    if training_params['TRUNK_FEATURE_EXPANSION'] > 0:
        processed_batch['xt'] = trunk_feature_expansion(
            processed_batch['xt'], training_params['TRUNK_FEATURE_EXPANSION']
        )
