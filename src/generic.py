...
def plot_histograms(*args):
    ...
    ax[0, 1].hist(y_pred, bins=1000, label=f'y_{{pred_{{test}}}}', color='darkorange', density=True)
    ax[0, 1].set_xlim([-2, 2])
    ax[0, 1].set_title(f'$y_{{pred}}$')
    ax[1, 0].hist(y_train, bins=1000, label=f'y_{{train}}', color='green', density=True)
    ax[1, 0].set_xlim([-2, 2])
    ax[1, 0].set_title(f'$y_{{train}}$')
    ax[1, 1].hist(y_val, bins=1000, label=f'y_{{val}}', color='green', density=True)
    ax[1, 1].set_xlim([-2, 2])
    ax[1, 1].set_title(f'$y_{{val}}$')
    ax[1, 2].hist(y_test, bins=1000, label=f'y_{{test}}', color='green', density=True)
    ax[1, 2].set_xlim([-2, 2])
...
def unnormalize(y, mean: float, std: float):
    return y*std + mean

def predict(model, X, y) -> np.ndarray:
    X = normalize(X)
    model.eval()
    if type(X) == list:
        X = np.array(X)
    ...
    with torch.no_grad():
        ...
        return unnormalize(prediction, mean, std).squeeze()
