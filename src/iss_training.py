...
# ---------------- Load training data -------------------
d = np.load(f"{path_to_data}/iss_train.npz", allow_pickle=True)
u_train, y_train, G_train =  load_data((d['X_branch'], d['X_trunk'], d['y']))
G_train  = G_train.reshape(-1,1)

d = np.load(f"{path_to_data}/iss_test.npz", allow_pickle=True)
...
y_dim = y_train.shape[-1]                           # Input dimension for trunk net -> q
...
train_dataset = ISSDataset(u_train, G_train)
test_dataset = ISSDataset(u_test, G_test)
...
ax[0].plot(x[1000:], [i.item() for i in train_loss_list][1000:], label='train')
ax[0].plot(x[1000:], [i.item() for i in test_loss_list][1000:], label='test', linewidth=0.5)
ax[0].set_xlabel('epoch')
ax[0].set_ylabel('MSE')
ax[0].set_yscale('log')
ax[0].legend()

ax[1].plot(x[1000:], train_err_list[1000:], label='train')
ax[1].plot(x[1000:], test_err_list[1000:], label='test', linewidth=0.5)
ax_1_sec = ax[1].twinx()
