...
from ...modules.utilities import dir_functions, plot_utils as plu
...
from ...modules.data_processing.data_loader import format_from_don

logger = logging.getLogger(__file__)

def format_from_don(arr: np.ndarray) -> tuple[np.ndarray]:
    """
    Recovers the original coordinate arrays from a DeepONet-formatted array.
...
def postprocess_for_2D_plot(output_data: dict[str, torch.Tensor], model: torch.NN.Module, plot_config: dict[str, any], model_config: dict[str, any]) -> dict[str, np.ndarray]:
    processed_data = {}
    ...
    branch_tuple = format_from_don(branch_features)
    ...
    coords_tuple = format_from_don(xt_plot)
